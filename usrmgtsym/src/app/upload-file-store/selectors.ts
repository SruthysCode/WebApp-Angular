...
import {
  ...
  const getError = (state: State): string => String(state.error);
  ...
  const getProgress = (state: State): number => Number(state.progress);
  
  const getInProgress = (state: State): boolean =>
    state.status === UploadStatus.Started &&Number(state.progress) >= 0;
